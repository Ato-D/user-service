package com.stlghana.admin_service.service.external;


import com.stlghana.admin_service.config.VOIPProperties;
import com.stlghana.admin_service.connector.KeycloakServiceConnectorClient;
...
import static com.stlghana.admin_service.utility.AppUtils.getResponseRecord;
import static com.stlghana.admin_service.utility.AppUtils.isNotNullOrEmpty;


@Service
@Slf4j
public record KeycloakService (
    ...
    /**
     * This method searches for role existence based on a search string.
     *
     * @param roleName The search string for user existence check.
     * @return The ID of the user if found.
     */
    public UUID findByRoleName(final String roleName) {
        return Objects.requireNonNull(keycloakServiceConnectorClient
                .searchForRoleByRoleName(properties.getKeycloak().getRealm(), roleName).getBody()
        ).get(0).getId();
    }
    ...
    public ResponseEntity<ResponseRecord> updateUser(UUID id, KeycloakUserDTO keycloakUserDTO) {
        ...
        try {
            ...
            List<KeycloakRoleDTO> assignedRoles = Objects.requireNonNull(keycloakServiceConnectorClient
                            ...
                            .getBody()).stream().filter(role -> !"default-roles-ess".equals(role.getName()))
                    .toList();

            keycloakServiceConnectorClient.deleteUserRoles(properties.getKeycloak().getRealm(),
                    id,assignedRoles);
            var record =
                    keycloakServiceConnectorClient.updateUser(properties.getKeycloak().getRealm(),id,
                            keycloakUserDTO).getBody();

            if (isNotNullOrEmpty(keycloakUserDTO.getUserRoles())) {
    ...
    /**
     * This method disables a user in the Keycloak service.
     *
     * @param id               The ID of the user to be disabled.
     * @param keycloakUserDTO  The DTO containing updated user information.
     * @return A ResponseEntity containing the updated user DTO.
     */
    public ResponseEntity<ResponseRecord> disableUser(UUID id, KeycloakUserDTO keycloakUserDTO) {
        ResponseRecord response;
        try {
            var record =
                    keycloakServiceConnectorClient.updateUser(properties.getKeycloak().getRealm(),id,
    ...
    public ResponseEntity<ResponseRecord> saveRole(final KeycloakRoleDTO keycloakRoleDTO) {
        ResponseRecord response;
        try {
            final ResponseEntity<KeycloakRoleDTO> keycloakResponse =
                    keycloakServiceConnectorClient
                            .saveRole(properties.getKeycloak().getRealm(), keycloakRoleDTO);
            log.info("keycloakResponse {}", keycloakResponse);
            response = getResponseRecord("Success", HttpStatus.valueOf(keycloakResponse.getStatusCode().value()), keycloakResponse.getBody());
    ...
    public ResponseEntity<ResponseRecord> updateRole(final KeycloakRoleDTO keycloakRoleDTO) {
        ResponseRecord response;
        try {
            final ResponseEntity<KeycloakRoleDTO> keycloakResponse =
                    keycloakServiceConnectorClient
                            .updateRole(properties.getKeycloak().getRealm(),
                                    String.valueOf(keycloakRoleDTO.getId()),
                                    keycloakRoleDTO);
            log.info("keycloakResponse {}", keycloakResponse);
            response = getResponseRecord("Success", HttpStatus.valueOf(keycloakResponse.getStatusCode().value()), keycloakResponse.getBody());
